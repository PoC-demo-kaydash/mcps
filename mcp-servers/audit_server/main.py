...
from mcp_tools.core.audit_tools import (
    GetAuditLogsTool,
    GetMyActivityTool,
    GetStatisticsTool
)
...
# 로거 설정
setup_logging()
logger = get_logger("audit_server")
...
class AuditServer:
    ...
    async def initialize(self):
        """리소스 초기화"""
        logger.info("=" * 60)
        logger.info("Audit Server Initializing...")
        logger.info("=" * 60)
        ...
        # Tool 초기화
        self.tools = {
            "get_audit_logs": GetAuditLogsTool(self.db, self.es),
            "get_my_activity": GetMyActivityTool(self.db, self.es),
            "get_statistics": GetStatisticsTool(self.db, self.es)
        }
